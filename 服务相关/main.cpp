...
{ 
    ...
    if( argc == 1 )
    {
        if (!StartServiceCtrlDispatcher( DispatchTable )) //分发函数
        { 
    ...
    {
        if( !GetModuleFileName(NULL, szPath, MAX_PATH) )//获取当前程序绝对路径
        {
        ...
        {
            InstallService(SVCNAME, szPath);//安装服务历程
            return;
        }
        else if( _tcsicmp(argv[1], L"uninstall") == 0 )
        {
            UninstallService(SVCNAME);//卸载服务
            return;
        }
        else
        {
